...
let ResourceFilename = "P.generated.swift"
...
func writeResourceFile(code: String, toFileURL fileURL: NSURL) {
    do {
        ...
        print("[P.swift] write error \(error.description)")
    }
}
...
struct Xcodeproj {
    ...
    init(url: NSURL) throws {
        ...
        guard let projectFile = try? XCProjectFile(xcodeprojURL: url) else {
            throw JSONParsingError.ParsingFailed("Project file at '\(url)' could not be parsed, is this a valid Xcode project file ending in *.xcodeproj?")
        }
    ...
    func resourceURLsForTarget(targetName: String, pathResolver: Path -> NSURL) throws -> [NSURL] {
        ...
        guard let target = allTargets.filter({ $0.name == targetName }).first else {
            ...
            throw JSONParsingError.ParsingFailed("Target '\(targetName)' not found in project file, available targets are: \(availableTargets)")
        }
...
struct CallInformation {
  ...
  init(arguments: [String], environment: [String: String]) throws {
    ...
    let commandName = arguments.first.flatMap { NSURL(fileURLWithPath: $0).lastPathComponent } ?? "pswift"
